...
class PositionalEncoding(tf.keras.layers.Layer):
    ...
    def call(self, input_len):
        # Ensure input_len is compatible with tf.gather
        input_len = tf.cast(input_len, dtype=tf.int32)
        
        # Generate positional indices tensor based on input_len
        max_len = tf.reduce_max(input_len)
        input_pos = tf.range(start=1, limit=max_len+1, delta=1)
        input_pos = tf.broadcast_to(input_pos, [tf.shape(input_len)[0], max_len])
        
        # Apply mask to zero out positions beyond each sequence's length
        mask = tf.sequence_mask(lengths=input_len, maxlen=max_len)
        input_pos = tf.where(mask, input_pos, 0)
        
        # Use tf.gather to select position encodings based on input positions
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, key, value, query, attn_mask=None,training = False):
        ...
        scale = (key.shape[-1] // self.num_heads) ** -0.5
...
class EncoderNew(tf.keras.Model):
    def __init__(self, vocab_size, max_seq_len, num_layers=1, model_dim=256, num_heads=4, ffn_dim=1024, dropout=0.0, is_public_dataset=False, l2_lambda=0.01):
        ...
        self.encoder_layers = [
            ...
            EncoderLayer(model_dim+ 1, num_heads, ffn_dim, dropout, l2_lambda) 
            for _ in range(num_layers)
        ]
        ...
        self.bias_embedding = tf.Variable(
            tf.random.uniform(shape=[model_dim+2], minval=-bound, maxval=bound),
            trainable=True
        ) if not is_public_dataset else  tf.Variable(
            tf.random.uniform(shape=[model_dim+1], minval=-bound, maxval=bound),
            trainable=True
        )
        # Positional Encoding remains unchanged
        self.pos_embedding = PositionalEncoding(model_dim + 2, max_seq_len) if not is_public_dataset else PositionalEncoding(model_dim + 1, max_seq_len)

        # Time Layer with L2 regularization
        self.time_layer = tf.keras.layers.Dense(model_dim + 2, kernel_regularizer=regularizers.l2(l2_lambda)) if not is_public_dataset else tf.keras.layers.Dense(model_dim+1, kernel_regularizer=regularizers.l2(l2_lambda))
...
class TransformerTime(tf.keras.Model):
    def __init__(self, n_event_code, batch_size,max_len, layer, dropout_rate, is_public_dataset=False, l2_lambda=0.01,model_dim = 256,name=None):
        ...
        self.feature_encoder = EncoderNew(n_event_code + 1, max_len + 1 , num_layers=layer,model_dim = model_dim,is_public_dataset=is_public_dataset, dropout=dropout_rate, l2_lambda=l2_lambda)
    ...
    def call(self, seq_input , training = False):
        ...
        mask_mult = tf.expand_dims(tf.math.logical_not(tf.cast(mask, tf.bool), tf.float32), axis=2) # 207,96,1
